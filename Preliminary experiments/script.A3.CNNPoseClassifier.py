...
if __name__ == "__main__":
    ...
    dataset_dir = os.path.abspath(config.get('CNNClassifier', 'dataset_dir'))
    batch_size = int(config.get('CNNClassifier', 'batch_size'))
    epochs = int(config.get('CNNClassifier', 'epochs'))
    log_dir = os.path.join(config.get('CNNClassifier', 'log_dir'))
    ...
    csv_train_file = os.path.join(dataset_dir, dataset_name + "-TRAIN.csv")
    csv_test_file = os.path.join(dataset_dir, dataset_name + "-TEST.csv")
    ...
    x_test = np.array(test_imgs[:5000])
    ...
    # ideal architecture configuration values:
    conv_layer = 2
    layer_size = 64
    dense_layer = 2
